...
async function handleTest() {
  ...
  try {
    ...
    if (results.value.length === 0) {
      message.info('未检索到符合条件的知识切片（建议适当调低【相似度阈值】或确认文件已解析）');
    } else {
      message.success(`成功检索到 ${results.value.length} 条命中结果`);
    }
  } catch (error: any) {
    console.error('检索测试失败:', error);
    message.error(error?.message || '检索测试请求失败，请检查服务状态');
  } finally {
    loading.value = false;
  }
}
    ...
    <Row :gutter="16">
      ...
      <Col :span="16">
        <Card title="检索结果" size="small" :bordered="true" class="h-full">
          ...
          <Alert
            v-if="isSearched && !results.length && !loading"
            message="未检索到符合条件的知识片段"
            description="建议解决方案：1. 尝试在左侧适当调低【相似度阈值】（如调整为 0.20 ~ 0.30）；2. 确保在【库内文件】页签中，已对上传的文件成功执行了【解析】操作。"
            type="warning"
            show-icon
            class="mb-4"
          />
          <Alert
            v-else-if="!isSearched && !results.length && !loading"
            message="请在左侧输入检索词，调整参数后点击【开始检索测试】"
            type="info"
            show-icon
            class="mb-4"
          />
